...
def gradient_attention_qwen2_5(image, prompt, general_prompt, model, processor):
    """
    Generates an attention map using gradient-weighted attention from Qwen2.5VL model.
    
    This function computes attention maps from the Qwen2.5VL model and weights them by their
    ...
    att_map = grad_att[0, :, -1, pos:pos_end].mean(dim=0).to(torch.float32).detach().cpu().numpy()
